...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray],
                                         cam: Union[str, np.ndarray], out_fname: str,
    ...
    if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(shape_original_after_cropping))]):
        ...
        cam_old_spacing = resample_data_or_seg(cam, shape_original_after_cropping, is_seg=False,
                                               axis=lowres_axis, order=order, do_separate_z=do_separate_z,
                                               order_z=interpolation_order_z)
    ...
    cam_out_fname = out_fname.split('.nii.gz')[0]+'_cam.nii.gz'
    cam_img = cam_old_spacing[0]
    cam_resized_itk = sitk.GetImageFromArray(cam_img.astype(np.float))
    cam_resized_itk.SetSpacing(properties_dict['itk_spacing'])
    cam_resized_itk.SetOrigin(properties_dict['itk_origin'])
    cam_resized_itk.SetDirection(properties_dict['itk_direction'])
    sitk.WriteImage(cam_resized_itk, cam_out_fname)
