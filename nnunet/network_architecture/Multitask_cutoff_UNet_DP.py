...
from nnunet.network_architecture.Multitask_cutoff_UNet import Multitask_cutoff_UNet
...
class Multitask_cutoff_UNet_DP(Multitask_cutoff_UNet):
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(Multitask_cutoff_UNet_DP, self).__init__(input_channels, base_num_features, num_classes, num_pool,
                                              num_conv_per_stage,
    ...
    def forward(self, x, y=None, return_hard_tp_fp_fn=False):
        res_seg, res_class = super(Multitask_cutoff_UNet_DP, self).forward(x)  # UNet forward pass   ##YLS

        if y is None:
            return res_seg, res_class
        else:
            # compute ce loss
            target_seg = [torch.remainder(t,10).long() for t in y]

            if self._deep_supervision and self.do_ds:
                ce_losses = [self.ce_loss(res_seg[0], target_seg[0]).unsqueeze(0)]
                tps = []
                fps = []
                fns = []

                res_softmax = softmax_helper(res_seg[0])
                tp, fp, fn, _ = get_tp_fp_fn_tn(res_softmax, target_seg[0])
                tps.append(tp)
                fps.append(fp)
                fns.append(fn)
                for i in range(1, len(target_seg)):
                    ce_losses.append(self.ce_loss(res_seg[i], target_seg[i]).unsqueeze(0))
                    res_softmax = softmax_helper(res_seg[i])
                    tp, fp, fn, _ = get_tp_fp_fn_tn(res_softmax, target_seg[i])
                    tps.append(tp)
                    fps.append(fp)
                    fns.append(fn)
                ret = ce_losses, tps, fps, fns
            else:
                ce_loss = self.ce_loss(res_seg, target_seg).unsqueeze(0)

                # tp fp and fn need the output to be softmax
                res_softmax = softmax_helper(res_seg)

                tp, fp, fn, _ = get_tp_fp_fn_tn(res_softmax, target_seg)
